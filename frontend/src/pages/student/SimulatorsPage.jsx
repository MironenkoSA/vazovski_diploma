...
const EXCEL_TASKS = [
  {
    id: 1,
    question: 'Посчитайте сумму бюджета по всем этапам проекта (столбец C)',
    hint: 'Используйте функцию СУММ()',
  ...
  },
  {
    ...
    hint: 'Используйте функцию СРЗНАЧ()',
...
const ExcelTrainer = ({ onClose }) => {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      <div style={{fontSize:13,color:'var(--muted)'}}>Задание {taskIdx+1} из {EXCEL_TASKS.length}</div>
...
const GANTT_TASKS_PRESET = [
  { id:1, name:'Инициация',     start:1,  duration:5,  color:'#FF6B6B' },
  { id:2, name:'Планирование',  start:4,  duration:10, color:'#FFB347' },
  { id:3, name:'Разработка',    start:12, duration:20, color:'#4ECDC4' },
  { id:4, name:'Тестирование',  start:30, duration:8,  color:'#7B68EE' },
  { id:5, name:'Внедрение',     start:36, duration:6,  color:'#51CF66' },
];
const TOTAL_DAYS = 45;

const GanttTrainer = ({ onClose }) => {
  const [tasks, setTasks] = useState([
    { id:1, name:'', start:1, duration:5 },
    { id:2, name:'', start:1, duration:5 },
    { id:3, name:'', start:1, duration:5 },
  ]);
  const [checked, setChecked] = useState(false);
  const [score, setScore]     = useState(0);
  const [showAnswer, setShowAnswer] = useState(false);

  const addTask = () => setTasks(p => [...p, { id: Date.now(), name:'', start:1, duration:3 }]);
  ...
  const checkAnswer = () => {
    ...
    tasks.forEach(t => {
      ...
      if (match) {
        if (Math.abs(match.start - t.start) <= 2) pts++;
  ...
  const renderBar = (task, idx, isAnswer=false) => {
    const left  = ((task.start - 1) / TOTAL_DAYS) * 100;
    ...
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <Card style={{background:'rgba(255,179,71,0.07)',border:'1.5px solid rgba(255,179,71,0.3)'}}>
        ...
        <div style={{fontSize:13,color:'var(--muted)'}}>
          Проект длится 45 дней. Добавьте этапы (Инициация, Планирование, Разработка, Тестирование, Внедрение),
          укажите день начала и продолжительность каждого.
        </div>
      </Card>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div style={{flex:1,display:'flex',justifyContent:'space-between',fontSize:11,color:'var(--muted)'}}>
          {[1,5,10,15,20,25,30,35,40,45].map(d=><span key={d}>{d}</span>)}
        </div>
      </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {tasks.map((t,i) => (
              <div key={t.id} style={{display:'flex',gap:6,alignItems:'center'}}>
                ...
                <input type="number" min="1" max="43" value={t.start}
                  onChange={e=>updateTask(t.id,'start',Math.max(1,parseInt(e.target.value)||1))}
                  style={{width:52,fontSize:13,padding:'6px 8px',textAlign:'center'}} title="День начала"/>
                <input type="number" min="1" max="45" value={t.duration}
                  onChange={e=>updateTask(t.id,'duration',Math.max(1,parseInt(e.target.value)||1))}
                  style={{width:52,fontSize:13,padding:'6px 8px',textAlign:'center'}} title="Продолжительность"/>
                <button onClick={()=>removeTask(t.id)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:16}}>×</button>
              </div>
            ))}
          </div>
          <div style={{fontSize:11,color:'var(--muted)',marginTop:6}}>Начало дня · Длительность дней</div>
